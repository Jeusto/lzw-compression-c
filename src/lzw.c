...
/*============================================================================*/
// FONCTIONS RELATIVES À LA STRUCTURE LISTE CHAINEE
/*============================================================================*/

ListeNoeud initialiser_liste(char* mode) {
  ListeNoeud dict = NULL;

  // Si le mode est "compression", on initialise le dictionnaire avec les 256
  // premiers cles (code ASCII) et valeurs (encodage hexadecimal correspondant)
  if (strcmp(mode, "compression") == 0) {
    ...
    // Sinon le mode est decompression on fait l'inverse
    for (int i = 0; i < 256; i++) {
...
void compresser_liste(ListeNoeud dict, FILE* fichier_source,
  ...
  /* Soit P le tout premier caractère lu dans le fichier, que nous transformons
   * en en chaine de caractères avant de le sauvegarder dans la variable
   * temporaire concatstr
   */
  ...
  /* Nous poursuivons la lecture tant que nous ne sommes pas à la fin du
   * fichier*/

  /* Soit C le caractère suivant.*/
  while ((lecture_char_C = fgetc(fichier_source)) != EOF) {
    concatstr = char2str(lecture_char_C);
    strcpy(cle_C, concatstr);
    free(concatstr);

    /* Nous concatenons les chaines de caractères P et C afin de vérifier si la
     * nouvelle chaine qui en résulte se trouve dans le tableau */
    concatstr = concat(cle_P, cle_C);
    strcpy(cle_P_plus_C, concatstr);
    free(concatstr);

    /* Recherchons la clé concaténée plus haut dans le dictionnaire. Deux cas de
     * figures sont à observer*/
    if ((strcmp(recuperer_liste(dict, cle_P_plus_C), "NULL")) != 0) {
      ...
      concatstr = concat(cle_P, cle_C);
      strcpy(cle_P, concatstr);
      /*Libérons la mémoire allouée par la fonction concat*/
      free(concatstr);

      /* Dans le cas contraire, la clé n'est pas retrouvée dans le dictionnaire.
       */
    } else {
      /* Nous récupérons alors la valeur associée à la clée récemment lue dans
       * le tableau
       */
      strcpy(valeur_P, recuperer_liste(dict, cle_P));
      wb_hex_as_short(fichier_destination, valeur_P);

      /* La nouvelle clée P + C est ensuite ajoutée dans le tableau
      ainsi que la valeur en héxadécimal qui lui correspond*/
...
void decompresser_liste(ListeNoeud dict, FILE* fichier_source,
  ...
  while (1) {
    // Si on a rempli le dictionnaire, on le vide et on recommmence a 0
    if (taille_actuelle_dico == TAILLE_DICT - 2) {
      liberer_liste(dict);
...
/*============================================================================*/
// FONCTIONS RELATIVES À LA STRUCTURE TRIE
/*============================================================================*/
...
void compresser_trie(TrieNoeud dict, FILE* fichier_source,
  ...
  while ((lecture_char_C = fgetc(fichier_source)) != EOF) {
    ...
    if ((strcmp(recuperer_trie(dict, cle_P_plus_C), "NULL")) != 0) {
      /*Si la clé est déja dans le dictionnaire, elle y est donc récupérée */
      strcpy(valeur_P_plus_C, recuperer_trie(dict, cle_P_plus_C));

      /* Concaténation des deux clés*/
      concatstr = concat(cle_P, cle_C);
      strcpy(cle_P, concatstr);
      /*Libérons la mémoire allouée par la fonction de concaténation*/
      free(concatstr);
      ...
      /* La nouvelle clée P + C est ensuite ajoutée dans le tableau
       ainsi que la valeur hexadécimale qui lui correspond*/
      taille_actuelle_dico++;
      sprintf(valeur_P_plus_C, "%08X", taille_actuelle_dico);
    ...
    /*Si le dictionnaire est rempli, il est vidé et réinitialisé*/
    if (taille_actuelle_dico == TAILLE_DICT - 2) {
      /* Output code for P */
      strcpy(valeur_P, recuperer_trie(dict, cle_P));
      wb_hex_as_short(fichier_destination, valeur_P);

      /*Nous libérons la mémoire allouée pour le dictionnaire avant de le
       * réinitialiser*/
      liberer_trie(dict);
...
/*============================================================================*/
// FONCTIONS RELATIVES À LA STRUCTURE HASHMAP
/*============================================================================*/
...
struct hashmap_s initialiser_hashmap(char* mode, char** tableau_cles,
  ...
  if (strcmp(mode, "compression") == 0) {
    ...
    // Sinon le mode est compression, on fait l'inverse
    for (int i = 0; i <= 255; i++) {
